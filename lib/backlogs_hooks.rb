        ...
        begin
          ...
          if issue.is_story?
            ...
            if Backlogs.setting[:story_points].blank?
              ...
              snippet += context[:form].select(:story_points, options_for_select(Backlogs.setting[:story_points].split(',').collect { |point| [point.to_s, point.try(:to_f)]}, issue.story_points.try(:to_f)), include_blank: true)
            end
            snippet += '</p>'
